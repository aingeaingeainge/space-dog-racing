...
| **v3** | `v3a`, `v3b`, `v3c`, `v3c2` | **`design/GDD_V3.md` 3.0, `design/BUILD_PLAN_V3.md`** |

Last reviewed: 23 September 2026. ⚠️ **`design/` mirrors in the claude.ai Project are three phases behind and need re-syncing.** Since the last sync: v3 Phase C2 added decision rows C12–C15 to `GDD_V3.md`, notes in §5.1, §5.3 (the hot pace replaces the cut contest rule), §5.6, §7.2, §11 and §14 Q10–Q11, and a Phase C2 section to `BUILD_PLAN_V3.md`; and before that, `GDD_V3.md` gained decision rows A1–A7 (v3 Phase A), B1–B9 (v3 Phase B) and C1–C11 (v3 Phase C), §2.1's calendar reads 8 regular planets, §12's `foodBand` note is expanded, and Phase C added inline status notes to §4.5, §5.1, §5.3 (the contest rule is cut), §5.5, §5.6, §7.2 and §11 and questions 9–11 to §14; `BUILD_PLAN_V3.md`'s Phase C gained A7 and a built-at-`v3c` status note, and §2.3 flags the `oddsScale` move; and `CANON.md` gained the write-once note above and the `v3b` and `v3c` tags.
