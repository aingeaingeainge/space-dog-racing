...
import { decideCrook, decideMixed, decideTrader, decideTrainer } from './paths';
 ...
 *
 * `careless`, `trainer`, `trader`, `crook` and `mixed` are measurement agents rather than
 * difficulties (BUILD_PLAN §7a.5) and are reachable only from the harness — nothing a player can
 * click offers them.
 */
export function decide(s: GameState, playerId: Id, agent: AiAgent = 'normal'): Action[] {
  switch (agent) {
    ...
    case 'crook':
      return decideCrook(s, playerId);
    case 'mixed':
      return decideMixed(s, playerId);
    case 'normal':
    default:
...
export { decideCrook, decideMixed, decideTrader, decideTrainer } from './paths';
