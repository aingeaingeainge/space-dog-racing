...
**Status: Phase A complete, tagged `v3a`.** Nine commits on `main` after `d0640ca` (`v2e`), **not pushed** —
...
| | |
|---|---|
| `146ff10` | Add the v3 design documents: CANON, GDD_V3, BUILD_PLAN_V3 |
| `3975718` | Mark v2's documents historical and point CLAUDE.md at CANON |
| `c3d80c1` | **Execute §2.1: delete the four markets, the debt system and the crook's road — snapshot move 1** |
| `280d7b0` | **Fold Trap into Acceleration: three stats, 0.40 / 0.35 / 0.25 — snapshot move 2** |
| `a356f33` | **Race or Rest, three purse tiers, ten weekends, an equal starting deal — snapshot move 3** |
| `cbd07d6` | Cut the harness down to v3 and add the three pace measures |
| `adbb2dc` | Sweep the §2.1 residue out of `packages/` |
| `e410b31` | Reject unknown harness flags instead of running the default silently |
...
## ⚠️ Read this first: three corrections to the record I made during this phase

The standing warning from Phase E was about producing *a number nobody can undo later*. I produced
three in this phase and caught them, and they are at the top because a reader who trusts the earlier
half of this session would be misled.
...
1. **The snapshot hashes in the first three commit messages were wrong when written.** They cited
   ...
   rewritten (the commits are unpushed, so only messages moved; every tree is byte-identical to
...
| Nothing in §2.1 remains referenced anywhere in `packages/` | ✅ | swept; see `adbb2dc` for what it found | **MET** |
...
`grep`-swept afterwards rather than assumed, which is `adbb2dc`; the sweep found the three
...
`championshipPoints` / `championshipTable` (live code, deleted in `a356f33`) and the whole
`trainThroughCheapWeeks` chain (deleted in `adbb2dc`). Worth recording because both survived a commit
...
⚠️ **Your `main` should be at `d0640ca` — that is `v2e` — with a clean tree.** (`ae95cd5` is `v2d`;
if that is where you are, merge `phase-e.bundle` first.)

**And read this before you fetch, if you merged `v3-design-docs.bundle` last session.** You would then
be at `c83debc`, and **that commit is not in this bundle's history.** The two design-doc commits were
re-created twice — once when I corrected the wrong snapshot hashes in the commit messages that follow
them, and again to sign every commit in the phase so GitHub shows them Verified — so they
have new hashes (`146ff10`, `3975718`) with byte-identical trees. Use the second block.

From `d0640ca` (`v2e`) — the ordinary case, and the one I round-trip tested:

```powershell
git status                                  # expect nothing modified
git log --oneline -1                        # expect d0640ca
...
git log --oneline -9                        # nine commits, these notes on top
```

If you are at `c83debc` (the design docs merged last session):

```powershell
git log --oneline -1                        # confirm c83debc before you do this
git reset --hard d0640ca                    # back to v2e; the bundle re-creates both doc commits
git fetch phase-v3a.bundle "refs/heads/main:refs/heads/v3a-work" "refs/tags/v3a:refs/tags/v3a"
git merge --ff-only v3a-work
git branch -d v3a-work
git log --oneline -9
...
⚠️ **Delete `v3-design-docs.bundle` from the folder afterwards** — it points at the old hashes and
fetching it again would give you two copies of the same two commits.
